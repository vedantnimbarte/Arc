  ...
  return (
    <div className="absolute inset-0 z-10 flex items-center justify-center bg-bg-base/95 backdrop-blur-sm animate-fade-in">
      <div className="flex max-w-[420px] flex-col items-center gap-3 rounded-window border border-status-err/30 bg-bg-panel/80 px-6 py-5 shadow-sheet">
        ...
        <div className="font-mono text-2xs uppercase tracking-widest2 text-status-err">
          connection failed
        </div>
        <pre className="max-h-[140px] overflow-auto whitespace-pre-wrap rounded-squircle bg-bg-subtle/60 px-3 py-2 font-mono text-xs text-fg-base">
          {message}
        </pre>
        <div className="mt-1 flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-squircle border border-border-subtle px-3 py-1.5 font-display text-xs text-fg-muted transition hover:bg-bg-hover hover:text-fg-base"
          >
            Close tab
          </button>
        </div>
