      ...
      {done ? (
        <div className="flex-1 overflow-y-auto px-4 py-3">
          <div className="rounded-squircle border border-status-ok/40 bg-status-ok/10 px-3 py-2">
            <div className="font-mono text-2xs uppercase tracking-widest2 text-status-ok">
              saved
            </div>
            <div className="mt-1 font-mono text-xs text-fg-base">~/.ssh/{done.name}</div>
            <div className="mt-1 font-mono text-2xs text-fg-muted">{done.fingerprint}</div>
          </div>
          ...
          <div className="mt-4">
            ...
            <pre className="max-h-[160px] overflow-auto rounded-squircle border border-border-subtle bg-bg-subtle/60 px-3 py-2 font-mono text-2xs leading-snug text-fg-base">
              {done.public}
            </pre>
            <button
              type="button"
              onClick={copyPub}
              className={cn(
                'mt-2 flex items-center gap-1.5 rounded-squircle border px-3 py-1 font-display text-xs transition',
                copied
      ...
      <div className="flex items-center justify-end gap-2 border-t border-border-subtle bg-bg-chrome/30 px-3 py-2.5">
        {done ? (
          <button
            type="button"
            onClick={onClose}
            className="rounded-squircle bg-accent/90 px-3 py-1.5 font-display text-xs text-bg-base transition hover:bg-accent"
          >
            Done
          </button>
        ) : (
...
function TypeChip({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'rounded-[8px] px-2.5 py-1 font-mono text-xs uppercase tracking-widest2 transition',
