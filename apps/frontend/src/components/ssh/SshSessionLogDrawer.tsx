  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 border-b border-border-subtle bg-bg-chrome/40 px-3 py-1.5">
        ...
        <div className="font-mono text-2xs text-fg-subtle">·</div>
        <div className="truncate font-display text-xs text-fg-base">{hostName}</div>
      ...
      <div
        ref={containerRef}
        className="flex-1 overflow-y-auto px-3 py-2 font-mono text-2xs leading-relaxed"
      >
...
export function LogDrawerToggle({ open, onToggle }: LogToggleProps) {
  return (
    ...
    >
      ...
      <span className="font-mono text-2xs uppercase tracking-widest2">Log</span>
    </button>
  );
}
    ...
    >
      <div className="flex items-center justify-between border-b border-border-subtle bg-bg-chrome/40 px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          <span className="font-display text-sm text-fg-base">Session log</span>
          {host && (
            <>
              <span className="font-mono text-2xs text-fg-subtle">·</span>
              <span className="font-mono text-xs text-fg-muted">{host.name}</span>
            </>
          )}
        </div>
        <button
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 font-mono text-2xs leading-relaxed">
        {!active && (
