...
export function PaneTabStrip({ paneId, variant = 'leaf' }: Props) {
  ...
  return (
    ...
    >
      {/* Tabs scroll; the pane controls below stay pinned to the right edge,
          so they don't slide out of reach once the strip overflows. */}
      ...
      {/* Split / maximize — standard mode still allows manual splits, and this
          strip replaces `PaneHeader`, which is where those controls otherwise
          live. Without them a split would only be reachable by edge-dropping. */}
      {variant === 'leaf' && (
        <div className="flex shrink-0 items-center gap-0.5 pl-1">
          <PaneControl
            ...
            onClick={() => {
              setFocusedPane(paneId);
              if (leaf.activeTabId) void splitPane(leaf.activeTabId, 'horizontal');
            }}
          />
          <PaneControl
            label="Split down"
            ...
            onClick={() => {
              setFocusedPane(paneId);
              if (leaf.activeTabId) void splitPane(leaf.activeTabId, 'vertical');
            }}
          />
          <PaneControl
...
/** One trailing icon button in the strip's pane-control cluster. Matches the
 *  sizing of `PaneHeader`'s controls so the two headers feel like one family. */
    ...
    <Tooltip label={label}>
      <button
        ...
        className={cn(
          ...
          'text-fg-subtle transition-colors duration-150 hover:bg-white/[0.07] hover:text-fg-base',
