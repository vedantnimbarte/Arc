...
| Usage stats       | ✅ V1          | Settings → **Usage** pane, two groups. **CLI Tools**: reuses `discover_ai_clis` to detect installed CLIs, then `usage_cli_fetch(id)` shells out to a hardcoded usage command per tool (`claude -p /usage`, `codex status --json`; 15s timeout) and returns output verbatim. Frontend `lib/usage.ts` (vitest) parses tokens/limits best-effort into cards; unparseable output / no-usage-command tools fall back to a raw block. **API Usage**: `usage_api_fetch(kind, apiKey, baseUrl)` → `arc_ai_runtime::fetch_usage` hits the Anthropic Admin / OpenAI org usage APIs (month-to-date). Needs an admin/usage-scoped key — a normal chat key maps 401/403 to a clear "not authorized" state. Fetches on pane open + per-card Refresh; no polling, nothing persisted. Arc's own usage is out of scope. |
