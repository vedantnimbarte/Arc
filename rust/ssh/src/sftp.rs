...
pub(crate) struct RemoteSession {
    ...
    forwards: Mutex<Option<SessionForwards>>,
}
...
impl SftpManager {
    ...
    pub async fn start_forwards(&self, id: &str, specs: Vec<ForwardSpec>) -> Result<()> {
        ...
        let forwards =
            SessionForwards::new(session.conn.handle.clone(), session.conn.remote_forwards.clone());
        for spec in specs {
        ...
        *session.forwards.lock().await = Some(forwards);
        Ok(())
    }
